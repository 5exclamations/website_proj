...
function schema(lang, page) {
  const graph = [
    ...
    { "@type": "MedicalBusiness", "@id": `${DOMAIN}/#practice`, name: "Vüsalə Qasımova — Neuropsychology", url: fullUrl(lang, "home"), telephone: "+994554770266", image: `${DOMAIN}${imageUrl(images.hero)}`, address: { "@type": "PostalAddress", streetAddress: "Aliyar Aliyev 39", addressLocality: "Baku", postalCode: "AZ1052", addressCountry: "AZ" }, areaServed: { "@type": "City", name: "Baku" }, openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"], opens: "09:00", closes: "18:00" }], amenityFeature: [{ "@type": "LocationFeatureSpecification", name: "Lift", value: true }], hasMap: "https://www.google.com/maps?q=Aliyar%20Aliyev%2039%2C%20Baku%2C%20Azerbaijan", sameAs: ["https://www.instagram.com/neyropsixoloq_qasimova_vusale"] },
...
function contactPage(lang) {
  ...
  return `<main id="main-content" class="contact-page"><section class="section"><div class="container two-cols contact-grid"><div><h1>${esc(d.title)}</h1><p>${esc(d.intro)}</p><address class="contact-details"><p><strong>${esc(d.phone)}</strong> <a href="tel:+994554770266">+994&nbsp;55&nbsp;477&nbsp;02&nbsp;66</a></p><p><strong>${esc(d.address)}</strong> Aliyar Aliyev 39, Baku 1052, Azerbaijan</p><p><strong>${esc(info.hoursLabel)}</strong> ${esc(info.hours)}</p><p><strong>${esc(info.landmarkLabel)}</strong> ${esc(info.landmark)}</p><p><strong>${esc(info.accessLabel)}</strong> ${esc(info.access)}</p><p>${esc(d.note)}</p></address><div class="contact-actions"><a class="btn btn-primary" href="${whatsappUrl}" target="_blank" rel="noopener">WhatsApp · +994 55 477 02 66</a><a class="text-link" href="https://www.google.com/maps?q=Aliyar%20Aliyev%2039%2C%20Baku%2C%20Azerbaijan" target="_blank" rel="noopener">${esc(d.openMap)} →</a></div><div class="map-card"><h2>${esc(d.locationTitle)}</h2><div class="map-frame"><iframe src="https://www.google.com/maps?q=Aliyar%20Aliyev%2039%2C%20Baku%2C%20Azerbaijan&amp;output=embed" loading="lazy" referrerpolicy="no-referrer-when-downgrade" title="${esc(d.locationTitle)}"></iframe></div></div></div><div class="card contact-form-card"><h2>${esc(site[lang].nav.book)}</h2>${form(lang, "contact")}</div></div></section></main>`;
}
